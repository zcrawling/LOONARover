...
function draw(s){
 ...
 const v=s.status?.values||{};set('battery',number(v['배터리 잔량 (%)'],'%'));set('temperature',number(v['Control MCU 온도 (°C)']??v['MCU 온도 (°C)'],' °C'));el('battery-fill').style.width=typeof v['배터리 잔량 (%)']==='number'?Math.max(0,Math.min(100,v['배터리 잔량 (%)']))+'%':'0%';
