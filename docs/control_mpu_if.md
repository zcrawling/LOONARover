...
이 문서가 MPU와 Control MCU 사이 UART byte stream의 유일한 wire 기준이다. MPU에서는
`mcu-io`만 이 link를 열며 cFS, ROS 2, GCS는 장치에 직접 접근하지 않는다. 목표는
...
Time sync, epoch, TTL, mode, trace, ACK, command result와 fault event는 v1 wire에 넣지 않는다.
그 기능이 필요해지면 기존 payload를 바꾸지 않고 protocol version을 올린다.
