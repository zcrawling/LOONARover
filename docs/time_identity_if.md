...
| `control_epoch` | command authority | `LNR_CTRL` | 이전 권한의 motion 차단 |
| `request_id` | discrete request | 최초 요청자 | dedup과 결과 연결 |
| `activity_id` | Payload/recovery activity | `LNR_CTRL` | 장시간 동작 상태 연결 |
...
- authority 획득, MANUAL session 교체, cFS 재시작 시 `control_epoch`을 새로 만든다.
- `valid_for_ms`는 0보다 크고 config 상한 이하여야 한다.
- 수신 시 이미 만료됐거나 epoch가 다르면 적용하지 않는다.
- reconnect와 mode 전이 시 latest command cache를 지운다.

MCU wire v1에는 epoch와 TTL을 싣지 않는다. `mcu-io`가 유효한 승인 명령만 최신값으로 전달하고, MCU는 `MPU_HEALTH` timeout으로 독립 정지한다.
...
| cFS 재시작 | `INIT`, 새 epoch, motion 없음 |
| `mcu-io` 재시작 | serial/link handshake 전 motion 없음 |
...
- GCS/cFS/mcu-io 개별 재시작 후 재실행 0건
