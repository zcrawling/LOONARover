...
| `CTL-005` | `mcu-io` kill | 500 ms watchdog로 MCU 정지 |
...
| `MAN-007` | cFS kill/restart | MCU 정지 후 `INIT` 재시작 |
...
| ID | 시험 | 통과조건 |
| --- | --- | --- |
| `LID-001` | Jazzy arm64 build/load | clean Pi 5에서 재현 |
...
| ID | 시험 | 통과조건 |
| --- | --- | --- |
| `SYS-001` | four-service kill matrix | 정의된 격리/정지 동작 일치 |
