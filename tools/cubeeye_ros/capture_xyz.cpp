...
public:
    ...
    std::string name() const override { return "loonar_cubeeye_xyz"; }
    ...
    void onCubeEyeFrameList(cu::ptr_source, const cu::sptr_frame_list& frames) override {
        ...
        for (const auto& frame : *frames) {
            if (frame->frameType() != cu::FrameType::PointCloud || frame->frameDataType() != cu::DataType::F32) continue;
            auto cloud = cu::frame_cast_pcl32f(frame);
            uint32_t header[] = {0x58595A31, static_cast<uint32_t>(frame->frameWidth()), static_cast<uint32_t>(frame->frameHeight())};
            size_t n = static_cast<size_t>(header[1])*header[2];
            uint64_t stamps[] = {static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()), frame->timestamp()};
            if (cloud->frameDataX()->size()!=n || cloud->frameDataY()->size()!=n || cloud->frameDataZ()->size()!=n) continue;
            if (!send_bytes(fd_,header,sizeof(header)) || !send_bytes(fd_,stamps,sizeof(stamps)) ||
                !send_bytes(fd_,cloud->frameDataX()->data(),n*4) || !send_bytes(fd_,cloud->frameDataY()->data(),n*4) ||
                !send_bytes(fd_,cloud->frameDataZ()->data(),n*4)) { stopping=1; return; }
            ++count;
        }
    }
};
int main(int argc, char** argv) {
    ...
    if (rc==cu::success) rc=camera->run(cu::FrameType::PointCloud);
    ...
    std::cerr << "XYZ frames: " << receiver.count << std::endl;
