...
void show(const loonar::gateway::Status& s) { const char* m=s.mode==Mode::kStop?"STOP":s.mode==Mode::kManual?"MANUAL":"AUTO"; std::cout<<"mode="<<m; if(s.last_received) std::cout<<" last="<<(s.last_received->source==Source::kManual?"manual":s.last_received->source==Source::kAuto?"auto":"stop")<<" linear="<<s.last_received->linear_mps<<" angular="<<s.last_received->angular_radps; std::cout<<'\n'; }
}
int main(int argc, char** argv) {
  if (argc < 3) { std::cerr << "usage: vehicle_gatewayctl {manual|stop|auto|monitor} SOCKET [LINEAR ANGULAR [DURATION_MS]]\n"; return 2; }
