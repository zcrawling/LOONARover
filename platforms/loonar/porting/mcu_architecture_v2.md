...
장치/세션 불일치와 명령 만료(backend 200 ms)에 대한 정지는 유지한다.
마지막 새 주행 명령 후 200 ms 이상 지나면 목표 속도를 0으로 한다.
RoboClaw ACK·조회 만료·오류는 피드백에 남기며 주행을 차단하지 않는다.
health는 상태 보고용이다. Pi에서 1초 미수신 시에도 보고만 하고 연결을 끊거나 STOP을 보내지 않는다.
