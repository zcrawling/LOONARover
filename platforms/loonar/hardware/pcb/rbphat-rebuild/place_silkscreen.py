...
add('RBPHAT B3',131,151.7,1.3);add('Pi5 / H16 / GAP19',131,154, .9)
add('CTRL',157,110.5,1);add('PAYLOAD',157,130.5,1)
add('BAT 0-12.6V',109,150, .9)
# References prioritized by package, then passive. Search nearest free space.
fs=sorted([f for f in b.GetFootprints() if f.GetReference() in bodies and not f.GetReference().startswith('H')],key=lambda f:(0 if f.GetReference()[0] in 'UJD' else 1,f.GetReference()))
for f in fs:
