#!/usr/bin/env python3
"""Independent assertions for the saved B3 layout and manufacturing handoff."""
...
fps={unq(next(p[2] for p in children(f,'property') if unq(p[1])=='Reference')):f for f in children(b,'footprint')};assert len(fps)==76
nn={};checked=0
for c in cs:
...
rows=list(csv.DictReader((P/'manufacturing/assembly/top-placement.csv').open()));assert len(rows)==62
assert {r['Ref'] for r in rows}=={c['reference'] for c in cs if not c['dnp'] and not c['reference'].startswith('TP') and c['reference']!='J1'}
...
report={'pcb_sha256':hashlib.sha256((P/'rbphat.kicad_pcb').read_bytes()).hexdigest(),'revision':'B3','schematic_pin_assignments_checked':checked,'erc_errors':0,'erc_warnings':0,'drc_violations':0,'unconnected_items':0,'schematic_parity_issues':0,'circuit_items':72,'mounting_holes':4,'fitted_smd_parts':62,'fitted_bottom_pth_socket':1,'dnp_parts':['R107','R207'],'testpoints':7,'track_segments':len(segs),'through_vias':len(vias),'trace_width_mm':[min(float(child(s,'width')[1]) for s in segs),max(float(child(s,'width')[1]) for s in segs)],'routing_layer_length_mm':dict(layerlength),'net_copper_length_mm':{n:round(le,4) for n,le in sorted(lengths.items())},'ground_planes':{'In1.Cu':'single connected filled island','In2.Cu':'single connected filled island'},'drc_ignored_checks':drc['ignored_checks'],'limitations':['CAD checks establish no detected layout/connectivity violations; no physical prototype or EMC testing performed.','2 Mbps target; no controlled-impedance fabrication requirement or 50 Mbps qualification claimed.','Nominal 19 mm mechanical stack needs first-article fit verification.']}
(P/'artwork/validation.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS B3: ERC 0/0, DRC 0, unrouted 0, parity 0, 240 pin assignments, 62 SMD + 1 socket')
