...
def library(name,pins,fp='',width=20.32):
    ...
    if name in ['GH5','THVD1451']:height=(max(len(ps) for ps in sides.values())+1)*10.16
    geom={}; ss=[]
    for side,ps in sides.items():
        for i,(num,label,typ,_) in enumerate(ps):
            pitch=10.16 if name=='GH5' or (name=='THVD1451' and side=='R') else 5.08
...
library('GH5',[(i,n,X,L) for i,n in enumerate(['TX+','TX-','RX+','RX-','GND'],1)],'Connector_JST:JST_GH_BM05B-GHS-TBT_1x05-1MP_P1.25mm_Vertical',15.24)
...
class Sheet:
 ...
 def comp(self,ref,kind,value,x,y,nets,mpn='',dnp=False,fp=None,source=''):
    ...
    self.items.append(f'(symbol (lib_id "RBPHAT:{kind}") (at {x} {y} 0) (unit 1) (in_bom {"no" if ref.startswith(("#","TP")) else "yes"}) (on_board {"no" if ref.startswith("#") else "yes"}) (dnp {"yes" if dnp else "no"}) (uuid {U(ref)}) {props} (instances (project "rbphat" (path {Q(path)} (reference {Q(ref)}) (unit 1)))))')
    ...
    for number,net in nets.items():
        ...
        shared=net in {'GND','POWER_GOOD','PI_SDA','PI_SCL','CTRL_TX','CTRL_RX','PAY_TX','PAY_RX'} or net.startswith('+3V3')
        if shared:
 ...
 def save(self):
    ...
    content=f'(kicad_sch (version 20250114) (generator "rbphat_rebuild") (uuid {self.id}) (paper "A3") (title_block (title {Q(self.title)}) (date "2026-09-05") (rev "B2") (company "LOONAR") (comment 1 "H16 socket / gap19 / Active Cooler; common ground; prototype - not fabrication release")) (lib_symbols {symbols}) '+ '\n'.join(self.items)
    if self.num==1:
...
s.note(20,20,'RBPHAT / REV B2    Dual full-duplex UART + 3S battery monitor',2.5)
...
for num,prefix,base in [(2,'CTRL',100),(3,'PAY',200)]:
 ...
 s.comp('J'+str(num),'GH5','BM05B-GHS-TBT',355,72.39,{1:n('TX+'),2:n('TX-'),3:n('RX+'),4:n('RX-'),5:'GND'},'BM05B-GHS-TBT(LF)(SN)')
 ...
 resistor(s,'R'+str(base+7),'120R / DNP',355,112,n('RX+'),n('RX-'),True)
