...
def astar(a,c,valid,vm):
 start=(0,round((a[1]-100)/step),round((a[0]-100)/step));end=(0,round((c[1]-100)/step),round((c[0]-100)/step));ez,ey,ex=end
...
for conn in drc['unconnected_items']:
 ...
 track(a,pt(path[0]),'F.Cu',net);nv=0
 ...
 track(pt(path[-1]),c,'F.Cu',net);print(net,a,'->',c,'vias',nv,flush=True);report.append({'net':net,'from':a,'to':c,'vias':nv})
