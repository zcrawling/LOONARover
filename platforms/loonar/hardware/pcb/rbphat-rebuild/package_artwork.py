...
writezip(M/'RBPHAT-B3-fabrication.zip',fab,M)
files=[f for f in P.iterdir() if f.is_file() and (f.suffix in ['.kicad_pcb','.kicad_pro','.kicad_sch','.kicad_sym','.kicad_dru','.md','.csv','.json','.py'] or f.name in ['fp-lib-table','sym-lib-table','rbphat.pdf','footprint-review.pdf','mechanical-review.pdf'])]
for d in ['RBPHAT.pretty','models','references']:files.extend(f for f in (P/d).rglob('*') if f.is_file())
for name in ['validation.json','final-drc.json','top-3d.png','bottom-3d.png','copper-layers.pdf','top-routing.svg','bottom-routing.svg','finish-connections.json','placement.json','silk-labels.json']:files.append(P/'artwork'/name)
...
writezip(M/'RBPHAT-B3-project.zip',files,P)
