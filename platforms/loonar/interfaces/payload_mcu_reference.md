# LOONAR Payload MCU Hardware Reference

상태: **물리 링크와 payload contract 기준자료**. cFS는 mission-level payload
activity를 소유하고, 별도 Payload Transport가 MCU link를 소유한다. 이 transport는
Vehicle Gateway와 분리되어 payload fault가 vehicle control에 영향을 주지 않게 한다.

## 1. 범위

Payload MCU는 sensor acquisition과 local task를 담당한다. MPU에서는 Payload
Transport만 serial link를 열고, cFS가 activity state와 mission command를 담당한다.
Payload 장애와 raw data burst는 Vehicle Gateway, Control link 및 주행을 막지 않는다.
...
cFS Mission Manager가 최종 `PayloadState`를 소유한다. MCU STATUS와 cFS activity가
불일치하면 새 동작을 시작하지 않고 상태를 `FAULT` 또는 `IDLE`로 명시적으로 재동기화한다.
...
Payload Transport의 선택적 writer는 고정 header + payload record를 rotating file에
순차 저장한다. writer queue는 byte 상한을 가지며 disk가 느리면 sample을 drop하고
counter를 증가시킨다. Vehicle Gateway control loop는 disk 완료를 기다리지 않는다.
...
GCS 요청은 `Ground Station -> cFS -> Payload Transport -> Payload MCU` 경로를 사용한다.
